...
OUTPUT_FILE="test_results_yices_$(date +%Y%m%d_%H%M%S).txt"
TMP_DIR="${TMPDIR:-/tmp}/btor2rw_yices_test_$$"
...
YICES=""
if command -v yices-smt2 &>/dev/null; then
    YICES="yices-smt2"
elif [ -x "/usr/local/bin/yices-smt2" ]; then
    YICES="/usr/local/bin/yices-smt2"
fi
...
if [ -z "$YICES" ]; then
    echo "Error: yices-smt2 not found."
    exit 1
fi

echo "=========================================="
echo "BTOR2 -> SMT2 Regression Test Suite (Yices)"
echo "=========================================="
echo "Binary:        $BTOR2RW_BIN"
echo "Test dir:      $TEST_DIR"
echo "bitwuzla:      $BITWUZLA (BTOR2 reference)"
echo "yices-smt2:    $YICES (SMT2 solver)"
...
for f in "${btor2_files[@]}"; do
    ...
    smt2_res=$(timeout -k 1s $TIMEOUT "$YICES" "$smt2_out" 2>/dev/null)
    smt2_rc=$?
    ...
    if [ $smt2_rc -ne 0 ]; then
        err_detail=$("$YICES" "$smt2_out" 2>&1 | head -1 | cut -c1-200)
        skip_smt2_error=$((skip_smt2_error + 1))
        smt2_error_files+=("$f -> $err_detail")
        printf "E"
        debug_stop "$f -> yices SMT2 error: $err_detail"
        continue
    fi
    ...
    if [ "$smt2_res" = "$btor2_res" ]; then
        ...
        fail=$((fail + 1))
        mismatch_files+=("$f -> BTOR2(bitwuzla)=$btor2_res SMT2(yices)=$smt2_res")
        printf "M"
        debug_stop "MISMATCH: $f -> BTOR2(bitwuzla)=$btor2_res SMT2(yices)=$smt2_res"
    fi
done
...
echo "=========================================="
echo "TEST RESULTS SUMMARY (Yices)"
echo "=========================================="
...
printf "  Skip (SMT2 timeout):  %3d  (yices)\n" $skip_smt2_timeout
printf "  Skip (SMT2 error):    %3d  (yices)\n" $skip_smt2_error
echo "=========================================="
...
if [ ${#smt2_error_files[@]} -gt 0 ]; then
    echo ""
    echo "SKIPPED (SMT2 solver error - yices):"
    echo "------------------------------------------"
...
cat > "$OUTPUT_FILE" << EOF
==========================================
BTOR2 -> SMT2 Regression Test Results (Yices)
Date: $(date)
Binary: $BTOR2RW_BIN
bitwuzla: $BITWUZLA (BTOR2 reference)
yices:    $YICES (SMT2 solver)
Test dir: $TEST_DIR
==========================================
Total files:        $total
  Pass:             $pass
