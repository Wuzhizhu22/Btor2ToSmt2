...
OUTPUT_FILE="test_results_z3_$(date +%Y%m%d_%H%M%S).txt"
TMP_DIR="${TMPDIR:-/tmp}/btor2rw_z3_test_$$"
...
for f in "${btor2_files[@]}"; do
    ...
    smt2_res=$(timeout -k 1s $TIMEOUT "$Z3" -smt2 "$smt2_out" 2>/dev/null)
    smt2_rc=$?
    ...
    if [ $smt2_rc -ne 0 ]; then
        err_detail=$("$Z3" -smt2 "$smt2_out" 2>&1 | head -1 | cut -c1-200)
        skip_smt2_error=$((skip_smt2_error + 1))
        smt2_error_files+=("$f -> $err_detail")
        printf "E"
        debug_stop "$f -> z3 SMT2 error: $err_detail"
        continue
    fi
    ...
    if [ "$smt2_res" = "$btor2_res" ]; then
        ...
        fail=$((fail + 1))
        mismatch_files+=("$f -> BTOR2(bitwuzla)=$btor2_res SMT2(z3)=$smt2_res")
        printf "M"
        debug_stop "MISMATCH: $f -> BTOR2(bitwuzla)=$btor2_res SMT2(z3)=$smt2_res"
    fi
done
...
printf "  Skip (SMT2 timeout):  %3d  (z3)\n" $skip_smt2_timeout
printf "  Skip (SMT2 error):    %3d  (z3)\n" $skip_smt2_error
echo "=========================================="
...
if [ ${#smt2_error_files[@]} -gt 0 ]; then
    echo ""
    echo "SKIPPED (SMT2 solver error - z3):"
    echo "------------------------------------------"
...
cat > "$OUTPUT_FILE" << EOF
==========================================
BTOR2 -> SMT2 Regression Test Results (Z3)
Date: $(date)
Binary: $BTOR2RW_BIN
