 ...
 *
 * 在 --no-or-assert 模式下，为每个bad生成独立的SMT2文件。
 ...
 *    --no-or-assert以及输入文件路径
 * 2. BTOR2加载：调用Btor2Loader::load_from_file将文件解析为Module结构
 * 3. 转换执行：根据选项调用Smt2Emitter生成SMT2格式输出
 *    - 默认模式：将全部bad通过or合并为一个assert后输出到单个文件
 *    - --no-or-assert模式：为每个bad节点生成独立的_badN.smt2文件
 ...
 * - --no-or-assert: 为每个bad生成独立文件（默认合并为一个assert）
 *
 * @param argc 命令行参数个数
 * @param argv 命令行参数数组
